...
/**
 * Route metadata (extracted at creation time)
 */
...
/**
 * Create type-safe client for define-route router
 *
 * @example
 * ```typescript
 ...
 *
 * // Query
 * const user = await api.getUser.query({ params: { id: '1' } });
 *
 * // Mutate with options
 * const created = await api.createUser.mutate(
 *   { body: { name: 'John' } },
 *   {
 *     headers: { 'X-Idempotency-Key': 'xxx' },
 *     onResponse: async (res, body) => {
...
): TypedClient<TRouter>
{
    ...
    function extractMetadata(routes: Record<string, any>, prefix = '')
    {
        for (const [name, route] of Object.entries(routes))
        {
            if ('handler' in route && route._method && route._path)
            {
                // It's a RouteDef
                const fullName = prefix ? `${prefix}.${name}` : name;
                routeMetadata.set(fullName, {
            ...
            else if ('routes' in route)
            {
                ...
                extractMetadata(route.routes, fullPrefix);
            }
        }
    }

    extractMetadata(router.routes);
    ...
    ): Promise<any>
    {
        ...
        if (options.cookies)
        {
            const cookieString = Object.entries(options.cookies)
                ...
                .join('; ');
            (init.headers as Record<string, string>)['Cookie'] = cookieString;
        }
 ...
 * ```
 */
export function configureApi<TRouter extends Router<any>>(
    router: TRouter,
    config: ApiConfig = {}
): void
{
    globalRouter = router;
    globalConfig = config;
    globalClient = null; // Reset client to force recreation
}

/**
 * Get global API client instance
 *
 * Throws if not configured via configureApi()
 */
export function getApi<TRouter extends Router<any>>(): TypedClient<TRouter>
{
    if (!globalRouter)
    {
        throw new Error(
            'API client not configured. Call configureApi(router, config) first.'
        );
    }

    if (!globalClient)
    {
        globalClient = createApi(globalRouter, globalConfig);
    }
 ...
 * ```
 *
 * Note: Must call configureApi() first
 */
