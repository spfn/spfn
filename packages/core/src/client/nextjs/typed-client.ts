...
/**
 * Re-export error classes for type-safe instanceof checks
 *
 * IMPORTANT: Always import error classes from here (not from @spfn/core/errors)
 * when using with createApi to ensure instanceof checks work correctly.
 *
 * @example
 * ```typescript
 ...
 *
 * try {
 *   await api.getUser.params({ id: '123' }).call();
 * } catch (error) {
