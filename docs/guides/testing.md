...
Test TypeBox schema validation:

```typescript
// src/lib/contracts/users.test.ts
import { describe, it, expect } from 'vitest';
...
import { findOne, create, update } from '@/lib/db/helpers';
...
describe('Database Helpers', () => {
  ...
  describe('update', () => {
    it('should update user', async () => {
      ...
      const updated = await update(users, { id: created.id }, {
        name: 'Updated Name',
      });
...
Test Hono routes with full contract validation:
...
> - **Contract tests**: Validate all schema definitions
